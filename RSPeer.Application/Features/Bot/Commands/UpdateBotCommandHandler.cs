...
{
	public class UpdateBotCommandHandler : IRequestHandler<UpdateBotCommand, Unit>
	{
		private readonly IMediator _mediator;
		private readonly RsPeerContext _db;
		private readonly IRedisService _redis;

		public UpdateBotCommandHandler(IMediator mediator, RsPeerContext db, IRedisService redis)
		{
			_mediator = mediator;
			_db = db;
			_redis = redis;
		...
		public async Task<Unit> Handle(UpdateBotCommand request, CancellationToken cancellationToken)
		{
			...
			{
				Key = $"bot:version:hash:{file.Version}",
				Value = hash
			}, cancellationToken);
